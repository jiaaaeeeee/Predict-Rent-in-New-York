...
set.seed(1031)
rf = ranger(price~minimum_nights_avg_ntm+minimum_nights+minimum_minimum_nights+zipcode+property_type+room_type+accommodates+bathrooms+bedrooms+cleaning_fee+guests_included+extra_people+availability_60+availability_365+review_scores_rating+review_scores_cleanliness+review_scores_location+instant_bookable,
                  data = data,
                  num.trees = 1000)

# Scoring ----
# Read scoring data and apply model to generate predictions
pred = predict(rf,data=scoringData)
# Construct submission from predictions
submissionFile = data.frame(id = scoringData$id, price = pred$predictions)
write.csv(submissionFile, '/Users/jiayihuang/Documents/APAN5200/rentlala2021/sample_submission8.csv',row.names = F)
